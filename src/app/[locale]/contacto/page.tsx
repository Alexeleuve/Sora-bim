...
import { Linkedin, CheckCircle2, Instagram, Facebook, MapPin, Mail } from 'lucide-react'
...
export default async function ContactPage({ params }: Props) {
  ...
  const whatsappHref = info.whatsappUrl
  ...
  return (
    <>
      <SchemaOrg schema={[getOrganizationSchema(), getBreadcrumbSchema([
      ...
      <SiteLayout showFloatingCTA={false}>
        ...
        <section className="bg-brand-800 pt-32 pb-16">
          <div className="container-section">
            ...
            <p className="font-sans text-lg text-white/65 leading-relaxed">{t('hero.subheadline')}</p>
          </div>
        </section>
        ...
        <section className="bg-neutral-50 section-py">
          <div className="container-section">
            <div className="grid grid-cols-1 lg:grid-cols-[5fr_7fr] gap-12 lg:gap-20 items-start">
              ...
              {/* Left: diagnosis + links */}
              <div>
                <ScrollReveal>
                  <div className="bg-white rounded-2xl p-8 border border-neutral-200 mb-6">
                    <h2 className="font-display font-bold text-neutral-900 text-xl mb-6">{t('diagnosis.headline')}</h2>
                    <ul className="space-y-4">
                      {diagItems.map((item, i) => (
                        <li key={i} className="flex items-start gap-3">
                          <CheckCircle2 size={18} strokeWidth={1.5} className="text-success-600 flex-shrink-0 mt-0.5" />
                  ...
                  <div className="space-y-2.5">
                    ...
                    >
                      <div className="w-9 h-9 rounded-lg bg-brand-50 flex items-center justify-center flex-shrink-0">
                        <Mail size={16} strokeWidth={1.5} className="text-brand-800" />
                      </div>
                    ...
                    >
                      <div className="w-9 h-9 rounded-lg bg-brand-50 flex items-center justify-center flex-shrink-0">
                        <WhatsAppIcon size={16} className="text-brand-800" />
                      </div>
                      <div className="min-w-0">
                        <p className="font-display font-semibold text-[0.6875rem] tracking-[0.06em] uppercase text-neutral-400 mb-0.5">WhatsApp</p>
                        <p className="font-sans text-sm text-neutral-800 group-hover:text-brand-500 transition-colors duration-150 truncate">{info.phone}</p>
                      </div>
                    </TrackedLink>

                    {/* Regular links (LinkedIn, Instagram, Facebook) */}
