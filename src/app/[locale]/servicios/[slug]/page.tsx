...
// ─── Slug maps ────────────────────────────────────────────────────────
// ES slugs → index in services.items array
const ES_SLUG_MAP: Record<string, number> = {
  'integracion-bim':    0,
  'coordinacion-bim':   1,
  'sistemas-especiales':2,
  'sistemas-electricos':3,
  'gestion-informacion':4,
  'bim-4d':             5,
  'bim-5d':             6,
}

// EN slugs → index in services.items array
...
function getIndex(locale: string, slug: string): number | undefined {
...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  const schemas = [
    ...
    getBreadcrumbSchema([
      ...
      { name: m.services.hero.label,    url: basePath },
...
export function generateStaticParams() {
  ...
  // EN service pages are generated by services/[slug]/page.tsx
  return Object.keys(ES_SLUG_MAP).map((slug) => ({ locale: 'es', slug }))
}
