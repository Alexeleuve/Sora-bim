...
export const metadata: Metadata = {
  ...
  openGraph: { type: 'website', locale: 'es_MX', url: SITE_CONFIG.url, siteName: SITE_CONFIG.name, images: [{ url: `${SITE_CONFIG.url}/images/og-default.jpg`, width: 1200, height: 630, alt: SITE_CONFIG.name }] },
...
export default async function LocaleLayout({ children, params }: Props) {
  ...
  return (
    <html lang={locale} className={`${montserrat.variable} ${inter.variable} ${ibmPlexMono.variable}`} suppressHydrationWarning>
      <head>
        <link rel="preconnect" href="https://fonts.googleapis.com" />
        <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="anonymous" />
        <meta name="msapplication-TileColor" content="#033C77" />
      </head>
      <body className="font-sans bg-neutral-50 text-neutral-700 antialiased overflow-x-hidden" suppressHydrationWarning>
        <a href="#main-content" className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[200] focus:bg-brand-800 focus:text-white focus:px-4 focus:py-2 focus:rounded-sm focus:font-display focus:text-sm focus:font-semibold">
